...
app.get("/list-companies", (req, res) => {
  ...
  db.query(`SELECT * FROM companies`, (err, resp) => {
    if (!err) {
      res.render("template/company/list-companies", {
        companies: resp,
        messages,
        status,
      });
...
app.get("/edit-company/:id", (req, res) => {
  ...
  db.query(`SELECT * FROM companies WHERE id = ${id}`, (err, resp) => {
    if (!err) {
      res.render("template/company/edit-company", {
        company: resp[0],
        messages,
        status,
      });
...
app.post("/edit-company/:id", (req, res) => {
  ...
  if (
    ...
    res.redirect(
      "/edit-company/" + id + "/?m=Įveskite kompanijos pavadinimą&s=danger"
    );
    return;
  }
  ...
  if (
    ...
    res.redirect(
      "/edit-company/" + id + "/?m=Įveskite kompanijos adresą&s=danger"
    );
    return;
  }
        ...
        if (dbresp[0].kiekis == 0) {
          db.query(
            ...
            (err, resp) => {
              if (!err) {
                res.redirect(
                  "/list-companies/?m=Įrašas sėkmingai išsaugotas&s=success"
                );
              } else {
                res.redirect("/list-companies/?m=Įvyko klaida&s=danger");
              }
